 ...
 * @author François
 */
public class CurseurScore extends Objet implements KeyListener {
	...
	private ScoreScreen s;	//Référence vers la carte
	...
	public CurseurScore(Game g, String sprite, int x, int y, PacDoge p, ScoreScreen s)
	{
		super(g,sprite,x,y);
		this.p = p;
		this.s = s;
		nOptions = s.getNOptions();
		numOption = 2;
		vSpacing = s.getVSpacing();
		hBlockOffset = s.getHBlockOffset();
		hOffset = s.getHOffset() - hBlockOffset;
		vBlockOffset = s.getVBlockOffset();
		vOffset = s.getVOffset() - vBlockOffset;
	}
